...
JOIN fk_id_cancion_id_genero fk ON c.id_cancion = fk.id_cancion
JOIN generos g ON fk.id_genero = g.id_genero
WHERE g.genero = 'Latin' AND c.anio = 2003;
...
JOIN fk_id_cancion_id_genero fk ON c.id_cancion = fk.id_cancion
JOIN generos g ON fk.id_genero = g.id_genero
WHERE g.genero IN ('Latin', 'Rap') AND a.biografia IS NOT NULL;
...
JOIN artistas a ON r.nombre_artista = a.nombre_artista
JOIN canciones c ON a.id_artista = c.id_artista
JOIN fk_id_cancion_id_genero fk ON c.id_cancion = fk.id_cancion
WHERE fk.id_genero = (SELECT id_genero FROM generos WHERE genero = 'Pop');
...
SELECT DISTINCT r.nombre_artista, g.genero, r.nombre_artista_similar
FROM relacion_artista_con_similares r
JOIN artistas a ON r.nombre_artista = a.nombre_artista
JOIN canciones c ON a.id_artista = c.id_artista
JOIN fk_id_cancion_id_genero fk ON c.id_cancion = fk.id_cancion
